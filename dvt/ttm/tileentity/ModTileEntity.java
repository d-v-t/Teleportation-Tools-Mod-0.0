...
public class ModTileEntity extends TileEntity implements IInventory {
	...
	public String getName() {
		return this.hasCustomName()?this.customName:"container.ttm_tile_entity"; 
	}
	...
	public ItemStack decrStackSize(int index, int count) {
		if (this.getStackInSlot(index) != null) {
			...
			if (this.getStackInSlot(index).func_190916_E() <= count) {
				is = this.getStackInSlot(index);
				this.setInventorySlotContents(index, null);
				this.markDirty();
				return is;
			} else {
				is = this.getStackInSlot(index).splitStack(count);
	...
	public void setInventorySlotContents(int index, ItemStack stack) {
		...
		if (stack != null && stack.func_190916_E() > this.getInventoryStackLimit()) {
			stack = new ItemStack(stack.getItem(), this.getInventoryStackLimit());
		}
